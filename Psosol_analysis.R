...
###############################INPUTS###########################
# Import working datasets
Psosol_data <- read_excel("Qol_Psoriasis.xlsx")
summary(Psosol_data)
PsoProve_RCT <- read_excel("PsoProve2_RCT dataset.xlsx")
summary(PsoProve_RCT)

#Combine multiple sources for unified datasets
# Join work_impair_0 from PsoProve_RCT dataset to Psosol_data dataset
...
# Initialize list to store value set
value_set <- list(
  mo2 = -0.069,
  mo3 = -0.314,
  sc2 = -0.104,
...
# Initialize list to store economic variables
l.inputs_psosol <- list(phototx_cost = 812,
...
# Function to estimate annual benefits 
...
md.pattern(Psosol_data)

#Explore demographics of study participants by sex
...
#Explore demographics of study participants by age
...
###############################HEALTH OUTCOME- PASI ESTIMATION###########################
...
###############################HEALTH UTILITY - QALY ESTIMATION###########################
####Explore Quality of life variables
#Before treatment started, who experienced the worst quality of life? by age and group
mo_0_counts <- Psosol_data %>%
  group_by(Group, mo_0) %>%
  summarise(Count = n()) %>%
  ungroup() 

#After treatment, who experienced the worst quality of life? by age and group
...
#Distribution plot of EQ-5D index by treatment group
...
#Estimate average work impairment
Psosol_data <- Psosol_data %>%
  mutate(
...
#Calculate income and benefits
Psosol_data <- Psosol_data %>%
  mutate(Ann_income_2022 = get_monthly_income(Age)* (hicp_2022 / hicp_2011) * 12)
...
###############################COMPARATIVE COST & UTILITY ESTIMATION###########################
# Initialize dataframe to store average cost and QALY 
